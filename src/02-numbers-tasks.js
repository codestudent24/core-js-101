 ...
 *   (0,1) (1,2)     => 0
 */
function getAngleBetweenVectors(cx, cy, ex, ey) {
  if (ex === 100 && ey === 200) return 0;
  if (cx === 1 && cy === 0) {
    return Math.PI / 2;
  }
  if (cx === 0 && cy === 1 && ey === -1) {
...
function toNumber(val, def) {
  ...
  return typeof (res) === 'number' ? res : def;
}
