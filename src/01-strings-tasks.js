 ...
 *   'ABABAB','BA' => 'ABAB'
 */
function removeFirstOccurrences(v1, v2) {
  const ind = v1.indexOf(v2);
  return v1.slice(0, ind) + v1.slice(ind + v2.length);
}
...
function getRectangleString(width, height) {
  ...
  let result = '┌';
  for (let i = 0; i < middleWidth; i += 1) {
    result += '─';
  }
  result += '┐\n';
  for (let i = 0; i < middleHeight; i += 1) {
    result += '│';
    for (let j = 0; j < middleWidth; j += 1) {
      result += ' ';
    }
    result += '│\n';
  }
